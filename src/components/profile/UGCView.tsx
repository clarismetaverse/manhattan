...
import { Pin, Briefcase, Users, Share2 } from 'lucide-react';
...
const UGCView: React.FC<ViewProps> = ({ profile }) => {
  const [openPinned, setOpenPinned] = useState<string | null>(null);
  const mode = 'UGC'; // views themselves don't switch styles; styling comes from parent

  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6">
      <div className="mt-6">
        <div className="text-xs mb-2 flex items-center gap-1 text-gray-500"><Pin className="w-3 h-3"/> Pinned Collabs</div>
        <div className="flex gap-3 overflow-x-auto pb-2">
          {pinned.map(t => (
            <div key={t.id} className="min-w-[160px] rounded-xl overflow-hidden shadow-lg cursor-pointer bg-white" onClick={() => setOpenPinned(t.id)}>
              <img src={t.cover} alt={t.title} className="w-full h-56 object-cover"/>
              <div className="p-2 text-sm">
                <div className="font-semibold truncate text-black">{t.code} — {t.title}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-6">
        <div className="text-xs mb-2 flex items-center gap-1 text-gray-500"><Briefcase className="w-3 h-3"/> Editorial / Marketing Projects</div>
        <div className="grid gap-4">
          {projects.map(p => (
            <div key={p.id} className="relative rounded-2xl overflow-hidden shadow-xl bg-white">
              ...
              <div className="p-3 text-sm relative z-10 backdrop-blur-sm bg-white/80">
                <div className="font-semibold text-base text-black">{p.title}</div>
      ...
      <div className="mt-8 text-center">
        <button className="px-6 py-3 rounded-xl shadow-lg flex items-center justify-center mx-auto bg-gradient-to-r from-pink-400 to-fuchsia-500 text-white shadow-pink-500/25">
      ...
      {openPinned && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-2xl shadow-2xl p-4 relative bg-white shadow-gray-500/30">
            <button className="absolute top-3 right-3 hover:opacity-75 text-gray-500" onClick={() => setOpenPinned(null)}>✕</button>
            {pinned.filter(p=>p.id===openPinned).map(p => (
              <div key={p.id}>
                <img src={p.cover} alt={p.title} className="w-full h-64 object-cover rounded-xl"/>
                <h3 className="text-lg font-bold mt-3 text-black">{p.code} — {p.title}</h3>
                <div className="text-sm mt-1 text-gray-600">{p.about}</div>
              </div>
            ))}
          </div>
        </div>
