...
type Service = { id: number; _actions_turbo?: ActionTurbo | null; [key: string]: unknown };
...
export default function VenueDetail({
  ...
  const [availabilityLoadingByOffer, setAvailabilityLoadingByOffer] = useState<Record<string, boolean>>({});
  const [availabilityErrorByOffer, setAvailabilityErrorByOffer] = useState<Record<string, string | null>>({});
  const [remainingByOffer, setRemainingByOffer] = useState<Record<string, Record<string, { remaining_slots: number }>>>({});
  const [availabilityRangeByOffer, setAvailabilityRangeByOffer] = useState<Record<string, { from: number; to: number }>>({});
  ...
  useEffect(() => {
    ...
    const cached = turboOffersCache.get(restaurantId);
    if (cached) {
      setTurboOffers(cached);
      setTurboOffersError(null);
      setTurboOffersLoading(false);
      return;
    }
  ...
  const galleryImages = useMemo(() => {
    const urls = restaurant?.GalleryRestaurant?.map((i) => i?.url).filter(Boolean) ?? [];
  ...
  const loadCalendarAvailability = useCallback(
    async (offerId: string, fromMs: number, toMs: number) => {
      ...
      if (cachedRange && cachedRange.from === fromMs && cachedRange.to === toMs && availabilityByOffer[offerId]) {
        return;
      }
      ...
      try {
        ...
        const payload = {
          ...
          offer_timeslot: raw.offer_timeslot.map(t => ({
        ...
        setAvailabilityRangeByOffer(prev => ({ ...prev, [offerId]: { from: fromMs, to: toMs } }));
        setAvailabilityLoadingByOffer(prev => ({ ...prev, [offerId]: false }));
        setAvailabilityErrorByOffer(prev => ({ ...prev, [offerId]: "Unable to load availability. Please try again." }));
      }
    },
      ...
      <div className="mx-auto max-w-sm pb-28">
        ...
        <AnimatePresence initial={false}>
          {!selectedOfferId && (
            ...
            >
              <h3 className="text-[15px] font-semibold text-stone-900 tracking-tight">Select a Collaboration</h3>
        ...
        {offersLoading ? (
            ...
            <div className="fixed inset-x-0 bottom-3 px-4">
              <div className="mx-auto max-w-sm rounded-2xl bg-white/70 backdrop-blur-lg ring-1 ring-white/50 p-2">
                <div className="px-2 pb-2 text-center text-xs text-stone-600">
                  {enabled ? (activeConfirmed ? `${offer.title} · ${activeConfirmed.timeLabel}` : offer.title) : "Pick an offer to continue"}
                </div>
                <motion.button
                  whileTap={{ scale: 0.98 }}
                  disabled={!enabled}
          ...
          <div className="mx-4 mt-6 rounded-2xl bg-white/70 p-4 text-sm text-stone-600">No offers available right now.</div>
        )}
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-4">
        {plates > 0 && (
          <div className="flex flex-col items-center text-center">
            <div className="flex h-[36px] w-[36px] items-center justify-center rounded-full bg-[#E8F5E9]/80">
              <PlateIcon />
            </div>
        ...
        {drinks > 0 && (
          <div className="flex flex-col items-center text-center">
            <div className="flex h-[36px] w-[36px] items-center justify-center rounded-full bg-[#FCE4EC]/80">
              <DrinkIcon />
            </div>
        ...
        {dessert > 0 && (
          <div className="flex flex-col items-center text-center">
            <div className="flex h-[36px] w-[36px] items-center justify-center rounded-full bg-[#FFF3E0]/80">
              <DessertIcon />
            </div>
        ...
        {champagne > 0 && (
          <div className="flex flex-col items-center text-center">
            <div className="flex h-[36px] w-[36px] items-center justify-center rounded-full bg-[#FFF8E1]/80">
              <ChampagneIcon />
            </div>
